...
Component({
  ...
  lifetimes: {
      ...
      return new Promise((resolve, reject) => {
        ...
        .then(({
          ...
          if (data.resmsg === '请求成功') {
            ...
            this.historyCitys = wx.getStorageSync('$city_choose_history') || [];
            this.setData(
              {
    ...
    attached() {
      ...
      return new Promise((resolve) => wx.getSystemInfo({
        success: resolve,
      }))
        .then(({
          windowHeight,
  ...
  methods: {
    ...
    confirm() {
      const { choosedCitys } = this.data;
      const findIndex = this.historyCitys.findIndex(
        ({
          code,
        }) => code === choosedCitys.code,
      );
     ...
     * 优化setData数据
     */
    diff: (key, arr) => arr.reduce((data, v, i) => Object.assign(data, {
      [`${key}[${i}]`]: v,
    }), {}),
  },
});
